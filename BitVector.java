...
public class BitVector
{
    ...
    {
        return !(index > 31) && (bits & (0x1 << index)) == (0x1 << index);
    }
    ...
    public String toPaddedBinaryString()
    {
        ...
        for (int i = 0; i <= 31; i++) {
            if ((bits & (0x1 << i)) != (0x1 << i)) {
                //will add to front of string and not to back of string
    ...
    {
        int bitSize = 0;
        for (int i = bitSize; i <= 31; i++) {
            //will adjust bitSize based on matching 1 at index (being set to 1)
            if ((bits & (0x1 << i)) == (0x1 << i)) {
                bitSize = i;
            }
        }
        //increased by 1 to accommodate for bit in 2^0 place
        return 1 + bitSize;
    }
}
